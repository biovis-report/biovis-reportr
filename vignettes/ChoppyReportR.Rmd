...
1. `get_color_func()` to get color function from BioVisReportR namespace.
2. `get_mode_lst()` to show all supported modes.
3. `get_discrete_pal_lst()` to show all supported palettes.
4. `get_continuous_pal_lst()` to show all supported palettes.
5. `get_continuous_palette()` show palette supported by Continuous Color Palettes.
6. `get_theme()` to auto generate theme for user.
...
p1_npg <- p1 + get_color_func(palname = 'npg', data_class='discrete', mode = 'color')()
p2_npg <- p2 + get_color_func(palname = 'npg', data_class='discrete', mode = 'fill')()
...
color palettes like [ColorBrewer](http://colorbrewer2.org) and [viridis](https://cran.r-project.org/package=viridis).
